...
class FindGroupTest < ActionDispatch::IntegrationTest
  def setup
    ...
    Membership.create(user_id: @user4.id, admin: true, group_id: @group3.id)
    Membership.create(user_id: @user4.id, admin: true, group_id: @group4.id)
  ...
  test 'Find groups: Filter= ada. returns 1 group (user is member of private groups)' do
